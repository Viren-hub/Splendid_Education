...
const getUnreadCount = async (req, res, next) => {
  try {
    const userId = req.user.id;
    // Fetch all relevant notifications and count those not read
    ...
    const unread = all.filter(n => !n.readBy.includes(userId)).length;
    res.status(200).json({ success: true, count: unread });
  } catch (error) { next(error); }
};
...
module.exports = { getMyNotifications, getUnreadCount, createNotification, deleteNotification, getAllNotifications, markAsRead, markAllAsRead };
