...
const getAllStudents = async (req, res, next) => {
  try {
    ...
    // Search by student ID on the Student table directly
    if (search) where.studentId = { [Op.iLike]: `%${search}%` };

    // Also match on user name or user email (OR across both tables)
    const userWhere = search
      ? { [Op.or]: [{ name: { [Op.iLike]: `%${search}%` } }, { email: { [Op.iLike]: `%${search}%` } }] }
      : undefined;

    // When searching, use OR between student ID and user fields
    const studentWhere = search
      ? { [Op.or]: [{ studentId: { [Op.iLike]: `%${search}%` } }, { '$user.name$': { [Op.iLike]: `%${search}%` } }, { '$user.email$': { [Op.iLike]: `%${search}%` } }] }
      : {};

    const findWhere = { isActive: where.isActive, ...(!search ? {} : { [Op.or]: [{ studentId: { [Op.iLike]: `%${search}%` } }] }) };
...
const updateMyProfile = async (req, res, next) => {
  try {
    ...
    const allowed = ['phone', 'address', 'emergencyContact', 'parentPhone', 'parentEmail'];
    const studentUpdate = {};
    allowed.forEach(f => { if (req.body[f] !== undefined) studentUpdate[f] = req.body[f]; });
    if (Object.keys(studentUpdate).length)
      await Student.update(studentUpdate, { where: { id: student.id } });
    ...
    const updated = await Student.findByPk(student.id, { include: fullInclude });
    res.status(200).json({ success: true, data: updated });
  } catch (error) { next(error); }
};
...
module.exports = {
  ...
  createStudent, updateStudent, deleteStudent, updateMyProfile,
