...
const getAllProjects = async (req, res, next) => {
  try {
    const data = await ProjectShowcase.findAll({ include: [studentInclude], order: [['createdAt', 'DESC']] });
    res.status(200).json({ success: true, count: data.length, data });
  } catch (error) { next(error); }
};
...
module.exports = { getPublishedProjects, getAllProjects, getProjects: getPublishedProjects, createProject, updateProject, deleteProject };
